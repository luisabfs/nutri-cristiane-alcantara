      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        <div className="flex justify-center md:justify-between items-center py-2 md:py-4 relative">
          ...
          <button
            ...
            className="flex items-center justify-center md:justify-start flex-shrink-0 relative z-20"
          >
            <Image
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <div className='space-x-2'>
              <button
                onClick={() => window.open('/comunidade-sozo', '_self')}
            ...
            <div className="fixed inset-0" onClick={(e) => e.stopPropagation()}>
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 mt-15 flex flex-col justify-center px-6 border-b-1 pb-8 pt-4 border-gray-50 bg-white/95 backdrop-blur-md shadow-lg rounded-b-2xl">
                  <nav className="space-y-6 items-center">
                    {navItems.map((item) => {
                      const isActive = activeSection === item.href.substring(1);
                      return (
                        <button
                          key={item.href}
                          onClick={() => scrollToSection(item.href)}
                          className={`block font-medium text-lg transition-all duration-300 text-center w-fit mx-auto ${isActive
                    ...
                    <div className="flex">
                      <button
                        onClick={() => window.open('/comunidade-sozo', '_self')}
                        className="mx-auto bg-gradient-purple-rose text-white font-semibold py-2 px-4 rounded-full transition-all duration-300 hover:scale-105"
                      >
                        Comunidade Sozo
                      </button>
                    </div>
